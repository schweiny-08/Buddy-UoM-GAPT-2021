...
namespace BuddyAPI
{
    public class AStar
    {
        ...
        public List<int> CalculateAStar(int startId, int endId)
        {
            ...
            {
                // retrieving tile with the lowest FCost
                ...
                foreach (var surrPin in surroundingPins)
                {
                    ...
                    if (notVisited.FirstOrDefault(l => l.map_long == surrPin.map_long
                            && l.map_lat == surrPin.map_lat) == null)
                    {
                        // compute its score, set the parent
                        surrPin.gcost = getGCost(start.map_long, start.map_lat, surrPin.map_long, surrPin.map_lat);
                        surrPin.hcost = getHCost(surrPin.map_long,
                            surrPin.map_lat, end.map_long, end.map_lat);
